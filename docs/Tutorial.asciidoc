...
1. Verify that your environment has proper credentials
   ...
   allow to select the location of the `.qiskitrc` file, and the
   stanza within it (e.g. `ibmq`) used for authentication.  Also,
   environment variables are available (resp. `QISKITRC` and
   `QISKIT_IDENTITY`) for these same.

2. Submit a Job

   We'll submit a version of the "Bell" circuit, and will specify a
   "user key" of "Bell3", which will get stashed in the local diary,
   so we can use that name further below.
